...
app.config["SQLALCHEMY_DATABASE_URI"] = 'sqlite:///'+os.path.join(basedir,'instance','project2.db')
...
def signup():
    ...
    existing_user = db.session.scalar(db.select(User).filter_by(email=data["email"]))
    if existing_user:
        return jsonify({"msg":"email taken bro"})
    hashed_pw = generate_password_hash(data["password"])
    
    try:
        new_user = User(email=data.get("email"), password_hash=hashed_pw)
